...
export abstract class Controller implements ControllerInterface {
  ...
  protected addStaticPath(data: UnknownObject): void {
    ...
    const staticPath = `${fullServerPath}/${this.configService.get('STATIC_DIRECTORY_PATH')}`;
    const uploadPath = `${fullServerPath}/${this.configService.get('UPLOAD_DIRECTORY')}`;
    transformObject(STATIC_RESOURCE_FIELDS, staticPath, uploadPath, data);
  }
